...
$("#login-form").submit(function(e){
    ...
    if(email=="")
    {
        ...
        $("#login-email").next(".help-block-error").html("Email cannot be blank.").css('color','red');
        check = 1;
    }
    if (!(isValidEmailAddress(email))) {
        $(".field-signupform-email").addClass("has-error");
        $("#login-email").next(".help-block-error").html("Enter a valid email").css('color','red');
        check = 1;    	
    }
    if(password=="")
    {
        $(".field-signupform-password").addClass("has-error");
        $("#login-password").next(".help-block-error").html("Password cannot be blank.").css('color','red');
        check = 1;    	
    }
    if(check==1)
    	return false;
	$.ajax({
		...
		success : function(data) {
			if ($.trim(data)=='empty') {
                ...
                $(".field-signupform-email").addClass("has-error");
                $("#login-email").next(".help-block-error").html("Invalid Account Credentials").css('color','red');
                cnt = 1;
            }
            else if ($.trim(data)=="passerr") {
                $(".field-signupform-password").addClass("has-error");
                $("#login-password").next(".help-block-error").html("Incorrect Password").css('color','red');
                cnt = 1;
            }            
...
function alterliststatus(status,listid) {
	$.ajax({
		type : 'POST',
		url : baseurl + '/alterliststatus',
        async: false,
		data : {
			status : status,
            listid : listid
...
$(document).ready(function(){
	$('.featurelist').change(function (){
		if(this.checked)
		{
			...
			alterfeaturelist('1',listid);
		}else{
			let listid = this.value;
			alterfeaturelist('0',listid);
		}
	});	
});


function alterfeaturelist( listid ) {
	let checkboxVal, status;
	checkboxVal = document.getElementById("featurelist_"+listid).checked;
...
function cancellationPageValidate() {
	...
	var cancelname = $('#cancellation-policyname').val();
	var cancelto = $('#cancellation-cancelto').val();
	var cancelpercentage = $('#cancellation-cancelpercentage').val();
	var canceldesc = $('#cancellation-canceldesc').val();
	...
	if($.trim(cancelname) == "") {
		$('.field-cancellation-cancelname').addClass('has-error');
		$('.field-cancellation-cancelname .help-block').html('Policy Name cannot be blank');
		return false;
	}

	if(cancelto <= 0) {
		$('.field-cancellation-cancelto').addClass('has-error');
		$('.field-cancellation-cancelto .help-block').html('Please enter valid day count');
		return false;
	}

	if(cancelpercentage < 10) {
		$('.field-cancellation-cancelpercentage').addClass('has-error');
		$('.field-cancellation-cancelpercentage .help-block').html('Cancel Percentage minimum should be 10');
		return false;   
	}

	if($.trim(canceldesc) == "") {
		$('.field-cancellation-canceldesc').addClass('has-error');
		$('.field-cancellation-canceldesc .help-block').html('Cancel description cannot be blank.');
		return false; 
	} 
...
function start_file_upload(idname)
{
    ...
    formdata.append('id', idname);
 	var stu_errorClass = "";
 	if(idname == "logoblack") {
 		stu_errorClass = ".logoimgerrcls";
 	} else if(idname == "logowhite") {
 		stu_errorClass = ".logoimgwhiteerrcls";
 	} else if(idname == "defaultuserimage") {
 		stu_errorClass = ".defaultusererrcls";  
 	} else if(idname == "favicon") {
 		stu_errorClass = ".faviconerrcls";  
 	}

 	if(file=="") {
		$(stu_errorClass).show();
		$(stu_errorClass).html('Please select image');
        setTimeout(function() {
                $(stu_errorClass).hide();
        },5000);		
	}
	...
	{
		if(fsize>=2) { 
	    	$(stu_errorClass).html("Image size upto 2 MB");
			setTimeout(function(){
				$(stu_errorClass).html('');
			}, 2000);     	
	    	return;
		...
		if(!(ftype == 'image/x-icon' || ftype == 'image/png') && idname == "favicon") 
		{
	    	$(stu_errorClass).html("Allowed format .ico and .png"); 
			setTimeout(function(){
				$(stu_errorClass).html('');
			}, 2000);     	
	    	return;    	
		} else if(ftype != 'image/jpeg' && ftype != 'image/jpg' && ftype != 'image/png' && idname != "favicon")
		{ 
	    	$(stu_errorClass).html("Allowed image format(.jpeg, .jpg, .png)"); 
			setTimeout(function(){
				$(stu_errorClass).html('');
			}, 2000);     	
	    	return;    	
		...
		if (formdata) {
			...
			$.ajax({
				...
				success: function (res) {  
					$("#"+idname+"upload").hide();
					if($.trim(res) == "size_error") { 
						$(stu_errorClass).html("Image size upto 2 MB");
						setTimeout(function(){
							$(stu_errorClass).html('');
						}, 2000);      	
				    	return; 
					} else if($.trim(res) == "type_error" && idname == "favicon") {
						$(stu_errorClass).html("Allowed image format(.ico, .png)"); 
						setTimeout(function(){
							$(stu_errorClass).html('');
						}, 2000);     	
				    	return;
...
function start_countryfile_upload(idname)
{
	...
	else
	{
	    ...
	    if(fsize>=2) { 
	    	$(".logoimgerrcls").html("Image size upto 2 MB");
			setTimeout(function(){
				$('.logoimgerrcls').html('');
			}, 2000);     	
	    	return;
		...
		{
	    	$(".logoimgerrcls").html("Allowed image format(.jpeg, .jpg, .png)"); 
			setTimeout(function(){
				$('.logoimgerrcls').html('');
			}, 2000);     	
	    	return;    	
...
function seoManagementValidate(){
...
var google_webmaster_link = $("#google_webmaster_link").val();
var facebook_link = $("#facebookLink").val();
...
if($.trim(sitetitle) == ""){
	...
	if($.trim(googleanalyticscode) == ""){
		$(".googleanalyticscodeerrcls").show();
		$(".googleanalyticscodeerrcls").html("Google analytics cannot be blank");
		$("#googleanalyticscode").keydown(function(){
			$(".googleanalyticscodeerrcls").hide();
			$(".googleanalyticscodeerrcls").html("");
		});
		return false;
	}
	if($.trim(google_webmaster_link) == ""){
		$(".google_webmaster_linkerrcls").show();
		$(".google_webmaster_linkerrcls").html("Google Webmaster cannot be blank");
		$("#google_webmaster_link").keydown(function(){
			$(".google_webmaster_linkerrcls").hide();
			$(".google_webmaster_linkerrcls").html("");
		});
		return false;
	}
	if($.trim(facebook_link) == ""){
		$(".facebooklinkerrcls").show();
		$(".facebooklinkerrcls").html("Facebook link cannot be blank");
		$("#facebookLink").keydown(function(){
			$(".facebooklinkerrcls").hide();
			$(".facebooklinkerrcls").html("");
		});
		return false;
	}
...
function manageCurrencyPriceValidate() {  
	var price = $("#currency-price").val();
	if($.trim(price) == "" || $.trim(price) <= 0 || $.trim(price) > 500) {
		$(".field-currency-price").addClass("has-error");
		$(".field-currency-price > .help-block").html("Currency value should be between 0.1 - 500");
    	$("#currency-price").keydown(function(){ 
        	$(".field-currency-price").removeClass("has-error");
       		$(".field-currency-price > .help-block").html(""); 
    	});
		return false;
	}
...
function createReportValidate(){
	var reportname = $("#profilereports-report").val();
	var reportshortdesc = $("#profilereports-shortdesc").val();

	/*if($.trim(reportname) == ""){
			$(".field-profilereports-report").addClass("has-error");
	    	$("#profilereports-report").next(".help-block").html("Report Name cannot be blank");
	    	$("#profilereports-report").keydown(function(){
	        	$(".field-profilereports-report").removeClass("has-error");
	       	$("#profilereports-report").next(".help-block").html("");
	    	});
			return false;
	} else {
		//$("#listcreatebtn").prop('disabled', true);
...
function createRoleValidate(){
	var rolename = $("#roleandprivilige-name").val();
	var roleshortdesc = $("#roleandprivilige-comment").val();
	var roles = $("input").val();
...
function updateRateValidate(){
	var rating = $("#reviews-rating").val();
	var reviews = $("#reviews-review").val();

	if($.trim(rating) == "" ){
			$(".field-reviews-rating").addClass("has-error");
	    	$("#reviews-rating").next(".help-block").html("Rating cannot be blank");
	    	$("#reviews-rating").keydown(function(){
	        	$(".field-reviews-rating").removeClass("has-error");
	       	$("#reviews-rating").next(".help-block").html("");
	    	});
			return false;
	}else if($.trim(reviews) == "")
	{
		$(".field-reviews-review").addClass("has-error");
	    	$("#reviews-review").next(".help-block").html("Review cannot be blank");
	    	$("#reviews-review").keydown(function(){
	        	$(".field-reviews-review").removeClass("has-error");
	       	$("#reviews-review").next(".help-block").html("");
	    	});
			return false;
	}else if($.trim(reviews) != "" && $.trim(reviews).length > 250) {
		$(".field-reviews-review").addClass("has-error");
	    	$("#reviews-review").next(".help-block").html("You have reached your maximum limit, only 250 characters are allowed");
	    	$("#reviews-review").keydown(function(){
	        	$(".field-reviews-review").removeClass("has-error");
	       	$("#reviews-review").next(".help-block").html("");
	    	});
			return false;
	} else if($.trim(rating) >  5)
	{
		$(".field-reviews-rating").addClass("has-error");
	    	$("#reviews-rating").next(".help-block").html("Rating cannot more than 5");
	    	$("#reviews-rating").keydown(function(){
	        	$(".field-reviews-rating").removeClass("has-error");
	       	$("#reviews-rating").next(".help-block").html("");
	    	});
...
var perctge = $("#commission-percentage").val();
	if($.trim(minval) == ""){
	 ...
	 if($.trim(maxval) == ""){
	  ...
	  if(parseInt(minval) > parseInt(maxval) || minval==maxval || minval<=0 || maxval<=0){
		$(".commerr").show();
	 ...
	 if($.trim(perctge) == "" || perctge <=0 || perctge > 100){
		$(".field-commission-percentage").addClass("has-error");
    	$("#commission-percentage").next(".help-block").html("The percentage should be between 1 - 99"); 
...
var perctge = $("#sitecharge-percentage").val();
	if($.trim(minval) == ""){
	 ...
	 if($.trim(perctge) == "" || perctge <=0 || perctge > 100){  
    	...
    	$("#sitecharge-percentage").next(".help-block").html("The percentage should be between 1 - 99");
...
//$("#bannerdesc").html($("#bannerdesc").val().replace(/\n/g, "<br>")); 
if($.trim(bannerdesc) == ""){
		$(".bannerdesccls").show();
...
if($.trim(supportdesc) == ""){
		$(".supportdesccls").show();
		$(".supportdesccls").html("Support Description cannot be blank.");
...
/* for currency purpose */
$(document).on('keypress',
		'#currency-price', function(evt) {
var charCode = (evt.which) ? evt.which : event.keyCode;
...
$( "#autoUptcurrency" ).change(function() {
 
  if($('#autoUptcurrency').is(':checked')){ var status=1; }
    else { var status=0; }

    $.ajax({
		type : 'POST',
		url : baseurl + '/Autoupdatecurrency',
        async: false,
		data : {
            status : status,
		},
...
function validatemoderatorform(){
	let firstname, lastname, email, password, check;

	firstname = $('#users-firstname').val();
	lastname = $('#users-lastname').val();
	...
	if(firstname == '')
	{
		$(".field-users-firstname").addClass("has-error");
		$("#users-firstname").next(".help-block").html("First name cannot be blank");
		check = 1;
	}

	if(lastname == '')
	{
		$(".field-users-lastname").addClass("has-error");
		$("#users-lastname").next(".help-block").html("Last name cannot be blank");
		check = 1;
	}

	if(email == '')
	{
		$(".field-users-email").addClass("has-error");
		$("#users-email").next(".help-block").html("Email address cannot be blank");
		check = 1;
	}

	if(password == '')
	{
		$(".field-users-password").addClass("has-error");
		$("#users-password").next(".help-block").html("Password cannot be blank");
		check = 1;
	}
	if(check = 1)
	{
...
function stripeSettingsValidate()
{
	var stripe_pk_key = $("#stripe_pk_key").val();
	var stripe_sk_key = $("#stripe_sk_key").val();
	var stripe_card = $("#stripe_card").val();
	var stripe_month = $("#stripe_month").val();
	var stripe_year = $("#stripe_year").val();
	var stripe_cvc = $("#stripe_cvc").val();
	var stripe_hostpaydays = $("#stripe_hostpaydays").val();

	var pk_check = 0, sk_check = 0;
	var cYear = (new Date()).getFullYear();
	var numeric = /^[0-9]*$/;

	var GivenDate = $.trim(stripe_year)+"-"+$.trim(stripe_month+"-01");
	var CurrentDate = new Date();
	GivenDate = new Date(GivenDate);

	if($.trim(stripe_pk_key) == ""){
		$(".stripe_pk_keyerrcls").show();
		$(".stripe_pk_keyerrcls").html("Stripe publishable key cannot be blank.");
		$("#stripe_pk_key").keydown(function(){
				$(".stripe_pk_keyerrcls").hide();
				$(".stripe_pk_keyerrcls").html("");
		});
		return false;
	}

	if (stripe_pk_key.indexOf('pk_test_') == 0 || stripe_pk_key.indexOf('pk_live_') == 0) {
		pk_check = 1;
	}

	if(pk_check == 0) {
		$(".stripe_pk_keyerrcls").show();
		$(".stripe_pk_keyerrcls").html("Invalid stripe publishable key");
		$("#stripe_pk_key").keydown(function(){
				$(".stripe_pk_keyerrcls").hide();
				$(".stripe_pk_keyerrcls").html("");
		});
	...
	if($.trim(stripe_sk_key) == ""){
		$(".stripe_sk_keyerrcls").show();
		$(".stripe_sk_keyerrcls").html("Stripe secret key cannot be blank.");
		$("#stripe_sk_key").keydown(function(){
				$(".stripe_sk_keyerrcls").hide();
				$(".stripe_sk_keyerrcls").html("");
		});
		return false;
	}


	if (stripe_sk_key.indexOf('sk_test_') == 0 || stripe_sk_key.indexOf('sk_live_') == 0) {
		sk_check = 1;
	}

	if(sk_check == 0) {
		$(".stripe_sk_keyerrcls").show();
		$(".stripe_sk_keyerrcls").html("Invalid stripe secret key");
		$("#stripe_sk_key").keydown(function(){
				$(".stripe_sk_keyerrcls").hide();
				$(".stripe_sk_keyerrcls").html("");
		});
	...
	if($.trim(stripe_card) == ""){
		$(".stripe_card_keyerrcls").show();
		$(".stripe_card_keyerrcls").html("Card number cannot be blank.");
		$("#stripe_card").keydown(function(){
				$(".stripe_card_keyerrcls").hide();
				$(".stripe_card_keyerrcls").html("");
		});
		return false;
	}

	if($.trim(stripe_card).length != 16){
		$(".stripe_card_keyerrcls").show();
		$(".stripe_card_keyerrcls").html("Card number must have 16 digits.");
		$("#stripe_card").keydown(function(){
				$(".stripe_card_keyerrcls").hide();
				$(".stripe_card_keyerrcls").html("");
		});
	...
	if($.trim(stripe_month) == ""){
		$(".stripe_expiry_keyerrcls").show();
		$(".stripe_expiry_keyerrcls").html("Card month cannot be blank.");
		$("#stripe_month").keydown(function(){
				$(".stripe_expiry_keyerrcls").hide();
				$(".stripe_expiry_keyerrcls").html("");
		});
		return false;
	}

	if($.trim(stripe_month).length > 2 || $.trim(stripe_month)<0 || $.trim(stripe_month)>12){
		$(".stripe_expiry_keyerrcls").show();
		$(".stripe_expiry_keyerrcls").html("Card month must be mentioned from 1 to 12");
		$("#stripe_month").keydown(function(){
				$(".stripe_expiry_keyerrcls").hide();
				$(".stripe_expiry_keyerrcls").html("");
		});
	...
	if($.trim(stripe_year) == ""){
		$(".stripe_expiry_keyerrcls").show();
		$(".stripe_expiry_keyerrcls").html("Card year cannot be blank.");
		$("#stripe_year").keydown(function(){
				$(".stripe_expiry_keyerrcls").hide();
				$(".stripe_expiry_keyerrcls").html("");
		});
		return false;
	...
	if($.trim(stripe_year).length != 4){
		$(".stripe_expiry_keyerrcls").show();
		$(".stripe_expiry_keyerrcls").html("Invalid card Year");
		$("#stripe_year").keydown(function(){
				$(".stripe_expiry_keyerrcls").hide();
				$(".stripe_expiry_keyerrcls").html("");
		});
	...
	if(GivenDate < CurrentDate) {
	    $(".stripe_expiry_keyerrcls").show();
		$(".stripe_expiry_keyerrcls").html("Invalid date, past date are not allowed");
		$("#stripe_year, #stripe_month").keydown(function(){
				$(".stripe_expiry_keyerrcls").hide();
				$(".stripe_expiry_keyerrcls").html("");
		});
		return false;
	...
	if($.trim(stripe_cvc) == ""){
		$(".stripe_cvc_keyerrcls").show();
		$(".stripe_cvc_keyerrcls").html("Card cvc cannot be blank.");
		$("#stripe_cvc").keydown(function(){
				$(".stripe_cvc_keyerrcls").hide();
				$(".stripe_cvc_keyerrcls").html("");
		});
		return false;
	}

	if($.trim(stripe_cvc).length < 3 || $.trim(stripe_cvc).length >4 ){
		$(".stripe_cvc_keyerrcls").show();
		$(".stripe_cvc_keyerrcls").html("Card cvc must have 3 or 4 digits number");
		$("#stripe_cvc").keydown(function(){
				$(".stripe_cvc_keyerrcls").hide();
				$(".stripe_cvc_keyerrcls").html("");
		});
	...
	if($.trim(stripe_hostpaydays) == ""){
		$(".stripe_hostpaydays_keyerrcls").show();
		$(".stripe_hostpaydays_keyerrcls").html("stripe due date to host pay cannot be blank.");
		$("#stripe_hostpaydays").keydown(function(){
				$(".stripe_hostpaydays_keyerrcls").hide();
				$(".stripe_hostpaydays_keyerrcls").html("");
		});
		return false;
	}

	if($.trim(stripe_hostpaydays).length < 1 || $.trim(stripe_hostpaydays).length >2 ){
		$(".stripe_hostpaydays_keyerrcls").show();
		$(".stripe_hostpaydays_keyerrcls").html("Host due days must have 1 or 2 digits number");
		$("#stripe_hostpaydays").keydown(function(){
				$(".stripe_hostpaydays_keyerrcls").hide();
				$(".stripe_hostpaydays_keyerrcls").html("");
		});
	...
	if($.trim(stripe_hostpaydays) < 2 || $.trim(stripe_hostpaydays) > 15){
		$(".stripe_hostpaydays_keyerrcls").show();
		$(".stripe_hostpaydays_keyerrcls").html("Host due days must have 2 to 15 days");
		$("#stripe_hostpaydays").keydown(function(){
				$(".stripe_hostpaydays_keyerrcls").hide();
				$(".stripe_hostpaydays_keyerrcls").html("");
		}); 
...
function cancel_order(reserveid) {
	$.ajax({
		type : 'POST',
		url : baseurl + '/admin/orders/cancelledorder',
        async: false,
		data : {
            reserveid : reserveid,
		},
		success : function(data) {
          	if ($.trim(data) == 1) {
                window.location = baseurl+"/admin/orders/completereservations";    	
            } else {
            	window.location.reload();
            }
        }
...
function claim_admin_order(reserveid, status)
{
	$.ajax({
		type : 'POST',
		url : baseurl + '/admin/orders/claimorder',
        async: false,
		data : {
            reserveid : reserveid,
            status : status
		...
		success : function(data) {
			...
			if(data == 1){
        		...
        		window.location = baseurl+"/admin/orders/completeclaim";
			}
        	else  
        		window.location.reload();
        }
...
$(document).ready(function () {
  ...
  $("#stripe_year, #stripe_card, #stripe_cvc, #stripe_hostpaydays").keypress(function (e) {
