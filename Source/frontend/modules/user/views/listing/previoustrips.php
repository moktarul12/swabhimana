...
<div class="bg_gray1">
  <div class="container">    
        <div class="col-xs-12 col-sm-3 margin_top20">
			...
			<?php
			echo '<li><a href="'.$baseUrl.'/user/listing/trips" class="active">'.Yii::t('app','Your Trips').'</a></li>            
            <li class="active"><a href="'.$baseUrl.'/user/listing/previoustrips">'.Yii::t('app','Previous Trips').'</a></li>';
			?>         
            </ul>
        ...
        <div class="airfcfx-panel panel panel-default">
          <div class="airfcfx-panel-padding airfcfx-panel panel-heading profile_menu1">
            <h3 class="panel-title"><?php echo Yii::t('app','Your Previous Trips');?></h3>
          </div>
						...
						echo '<table class="table table_no_border tripdesign">
                                	<thead>
                                      <tr class="review_tab">
										...
										<th>'.Yii::t('app','Checkin Out Date').'</th>
					...
					foreach($trips as $trip)
					{
                     ...
                     $listinghours_end=date("h:i A", strtotime($listinghours[1]));
						...
						$resized_userimage = Yii::$app->urlManager->createAbsoluteUrl ('resized.php?src='.$userimage.'&w=40&h=40');	
							...
							if($trip->booking=='perhour'){
								$hours=explode('*|*',$trip->hourly_booked);
							...
							if($trip->booking=='pernight' && $listdata->pernight_availablity!="" && $listdata->pernight_availablity!=null)
							{
							...
							echo'<div class="datetd"><label>Location : </label><span>';


							if(isset($listdata->accesscode) && !empty($listdata->accesscode))
								echo $listdata->accesscode.', ';
							echo $listdata->streetaddress.', '.$listdata->city.',</span> <span>'.$listdata->state.', '.$listdata->zipcode.'</span></div><div class="datetd"><label>Host : </label><span>'.$hostdata->firstname.' '.$hostdata->lastname.'</span></div>';							
							...
							{
								echo '<td>'.$trip->checkin.'<br/>'.$trip->checkout.'</td>';
							}
							else
							echo '<td>--</td>';
							...
							{
								if($trip->checkin!='0000-00-00 00:00:00' && $trip->checkout!='0000-00-00 00:00:00' && strtotime($trip->checkout) < strtotime($current_Timezone))
								{   
									echo '<td><div id="reviewbtn'.$trip->id.'"><a href="javascript:void(0);" data-toggle="modal" data-target="#reviewpopup"><input type="button" value="'.Yii::t('app','Review').'" class="btn btn-danger" onclick="review_trip('.$trip->id.')"></a></div></td>';
								} 
								else
									echo '<td>--</td>';
							}
							else if(in_array($trip->bookstatus,$reserve_status) || ($trip->bookstatus == "refunded" && strtolower($trip->cancelby) == "admin")) 
							{
								...
								echo '<td align="center"><a href="'.$reviewurl.'" class="airfcfx-red-font">'.Yii::t('app','View Review').'</a></td>';								
							} else {
								echo '<td align="left">--</td>';  
							} 

							echo '<td clas=="airfcfx-min-width-80px">';
							if($trip->bookstatus!="")
							{
...
<div class="modal fade" id="reviewpopup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog emergency_width" role="document">
    <div class="modal-content">
      <div class="modal-header clearfix no_border">
