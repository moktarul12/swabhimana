...
io.sockets.on( 'connection', function( client ) { 
	...
	client.on( 'message', function( data ) {
		console.log( 'inquiry ' + data.inquiryId + 'receiver ' + data.receiverId + " sender :" +data.senderId + " message :" +data.message +" Listing :"+data.listingId + 'chat type' + data.chat_type );
		
		//io.sockets.emit( 'message', { receiver: data.receiverId, sender: data.senderId, listing: data.listingId, message: data.message } );

		io.sockets.in('/normal/'+ data.inquiryId ).emit( 'message', { inquiry: data.inquiryId, receiver: data.receiverId, sender: data.senderId, listing: data.listingId, message: data.message,chat_type : data.chat_type  } );
	});
