	...
	public function actionAddbuttonslider()
{
    ...
    if (Yii::$app->request->isPost) {
        ...
        $path = Yii::getAlias('@webroot/images/buttonsliders/');
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
        }

        $file = UploadedFile::getInstance($model, 'sliderimage');
        ...
        $model->title       = Yii::$app->request->post('Buttonsliders')['title'];
        $model->description = Yii::$app->request->post('Buttonsliders')['description'];
        $model->buttonname  = Yii::$app->request->post('Buttonsliders')['buttonname'];
        $model->buttonlink  = Yii::$app->request->post('Buttonsliders')['buttonlink'];
