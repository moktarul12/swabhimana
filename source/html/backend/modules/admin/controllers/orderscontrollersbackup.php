...
class OrdersController extends Controller
{
   ...
   public function actionClaimorder()
   {
     ...
     $flag = 0;
      ...
      if($reservation != null && empty($reservation->claim_transaction) && $reservation->bookstatus == "claimed" && $reservation->claim_status == "pending" && $hostdata->stripe_status == "verified" && $reservation->securityfees > 0 && !empty($reservation->securityfees)) {
         ...
         if($status=="approve") {
            ...
            $stripe = new \Stripe\StripeClient(
              ...
              $chargeJson = \Stripe\PaymentIntent::create([
                ...
                'stripe_account' => $host_account_id['accountid'],
              ]);
              
            $striperesult = $chargeJson->jsonSerialize();
            $result = array();
            if (!empty($striperesult['id'])) { 
