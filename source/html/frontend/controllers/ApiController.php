...
public function actionGetconversation()
	...
	public function actionAdmindata()
	{
		...
		if (!empty($sitesettings)) {
			...
			$stripe_secret_key = $sitesettings->stripe_secretkey;
			...
			// echo '<pre>'; print_r($account_result['id']); die;
			if(!empty($account_result['id']))
			{
			...
			$data = array(
				...
				'return_url' => Yii::$app->urlManager->createAbsoluteUrl("connecthostaccounts").'?code='.$account_id,
				'refresh_url' => Yii::$app->urlManager->createAbsoluteUrl("connecthostaccounts").'?code='.$account_id,
			);
			...
			$resultarray['connect_url'] = $connectUrl;
