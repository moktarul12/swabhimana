...
<div class="bg_gray1">
      ...
      if($model->bookstatus == "accepted" && $model->orderstatus == "pending") {
          ...
          $hostAmount = number_format(round(($hostAmount),2),2,".","");
    ...
    ,'VUV','XAF','XOF','XPF'];
    $stripecurrency = $model->currencycode;
    if(in_array(strtoupper(trim($stripecurrency)),$stripe_currency)){
      $nightprice = $model->pricepernight * $totalduration;
      $nightprice = number_format(round($nightprice));  
      $commissionfees = number_format(round($rate * $model->commissionfees));
      $servicefees = number_format(round($rate * $model->servicefees));
      $securityfees = number_format(round($rate * $model->securityfees));
      $taxfees = number_format(round($rate * $model->taxfees));
      $cleaningfees = number_format(round($rate * $model->cleaningfees));
      $sitefees = number_format(round($rate * $model->sitefees));
      $totalprice = number_format(round(($rate * $model->total))); 
    }
    else{
      $nightprice = $model->pricepernight * $totalduration;
      $nightprice = number_format(round($nightprice,2),2,".","");  
      $commissionfees = number_format(round($rate * $model->commissionfees,2),2,".","");
      $servicefees = number_format(round($rate * $model->servicefees,2),2,".","");
      $securityfees = number_format(round($rate * $model->securityfees,2),2,".","");
      $taxfees = number_format(round($rate * $model->taxfees,2),2,".","");
      $cleaningfees = number_format(round($rate * $model->cleaningfees,2),2,".","");
      $sitefees = number_format(round($rate * $model->sitefees,2),2,".","");
