            		...
            		$reserveTotal = round(($rate * $reservation->total),2);  
						...
						$resized_userimage = Yii::$app->urlManager->createAbsoluteUrl ('resized.php?src='.$userimage.'&w=40&h=40');	
							...
							// Update_16March2023
								...
								if(($booking_type == 'perhour' && $checkout_datetime < time() || $booking_type == 'pernight' && $todate < $today) && $reservation->bookstatus != "declined" && $reservation->bookstatus!="refunded" && $reservation->bookstatus!="claimed" && isset
								($reservation->securityfees) && $reservation->securityfees != "" && $reservation->securityfees > 0)
								{
